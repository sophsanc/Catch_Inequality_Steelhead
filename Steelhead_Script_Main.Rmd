...
## % Entries excluded for incomplete data (see above)
1 - (nrow(SRRC_raw_clean) / nrow(SRRC_raw))
...
Num_anglers_annual <- SRRC_all %>% 
  group_by(Year) %>% 
  summarise(Unique_anglers = n_distinct(`Angler ID`))
...
SRRC_filtered_ALL <- SRRC_all %>% # DF of SRRC from only approved basins
...
## Anglers catching zero fish (all basins)
No_fish<- SRRC_all %>%
  group_by(Year, `Angler ID`) %>%
  summarise(
    Annual_catch = sum(T_sum, na.rm = TRUE)) %>%
  ungroup() %>% 
  group_by(Year) %>%
  summarise(
    Total_anglers = n(),
    Zero_anglers = sum(Annual_catch == 0),
    Prop_zero = Zero_anglers / Total_anglers)

mean(No_fish$Prop_zero) ## Annual avg of anglers catching zero fish (all basins)
...
63460/121505 ## % angler cards reporting zero fish catch (all basins)

# Anglers catching zero fish (filtered basins) --> use this since we are comparing to Gini/LAC, which only accounts for filtered basins
No_fish_filtered <- SRRC_filtered_ALL %>%
  group_by(Year, `Angler ID`) %>%
  summarise(
    Annual_catch = sum(T_sum, na.rm = TRUE)) %>%
  ungroup() %>% 
...
mean(No_fish_filtered$Prop_zero) ## Annual avg of anglers catching zero fish all year; regardless location (filtered basins)

No_fish_filtered_LocYear <- SRRC_filtered_ALL %>%
  group_by(Year, LocName, `Angler ID`,Region) %>%
  summarise(
    Annual_catch = sum(T_sum, na.rm = TRUE)) %>%
  ungroup() %>% 
  group_by(Year, LocName,Region) %>%
  summarise(
    Total_anglers = n(),
    Zero_anglers = sum(Annual_catch == 0),
    Prop_zero = Zero_anglers / Total_anglers)

No_fish_filtered_Loc <- No_fish_filtered_LocYear %>% 
  group_by(LocName,Region) %>%
  summarise(
  Mean_prop_zero_anglers = mean(Prop_zero))

No_fish_filtered_Loc %>% 
  group_by(Region) %>% 
  summarise(Mean_region_zero_anglers = mean(Mean_prop_zero_anglers))
...
59570/115926 ## % angler cards reporting zero fish catch (filtered basins)
...
# png("teachingfigure2A.png", width = 6, height = 6, units = "in", res = 300)
# par(mar = c(5, 5.5, 4, 2)) #fixes cut off axis titles when exporting

plot(lc_trinity_2017, 
     main = "",
     ylab = "Cumulative % Fish Caught", 
     xlab = "Cumulative % Anglers",  #leave blank if exporting together
     cex.lab = 1.3,
     col = "darkslategray",
     lwd = 3, 
     frame.plot = FALSE)
...
text(x = 0.5, y = 0.55, labels = "Line of Equality (1:1)", srt = 44, col = "black", cex = 1.2)
...
#dev.off()  # For single plot export 

## Example Figure 2B

# png("teachingfigure2B.png", width = 6, height = 6, units = "in", res = 300)
# par(mar = c(5, 5.5, 4, 2)) #fixes cut off axis titles when exporting
plot(lc_american,
     main = "",
     ylab = "Cumulative % Fish Caught", #leave blank if exporting together  
     xlab = "Cumulative % Anglers",  #leave blank if exporting together  
     cex.lab = 1.3, #leave out if exporting together
     col = "goldenrod3", 
     lwd = 3,
     frame.plot = FALSE)
...
lines(lc_trinity, col = "coral3", lwd = 3)
abline(0, 1, lty = 2, lwd = 3, col = "black")

legend("topleft", 
       legend = c("American R. (2017),  Gini = 0.90", "Trinity R. (2017),  Gini = 0.72"),
       col = c("goldenrod3", "coral3"),
       lwd = 3,
       cex = 1.2,
       bty = "n")

#dev.off() #if exporting individually

## Shared x-axis, if exporting together
#mtext("Cumulative % Anglers", side = 1, outer = TRUE, line = 1, cex = 1.4,  adj = 0.53)
#dev.off() if exporting together
...
#  geom_hline(yintercept = NC_Gini_Mean, color="coral4", linewidth = 1) +
  ...
  annotate("text", x = 13, y = 0.95, label = "Other", size = 6.3, color = "#0A484F", fontface = "bold") 
barchart_T_Gini_ALL
...
LMM_plot1 <- LMM_T_Gini_ALL %>% 
  ggplot(aes(x = LogCardCount, y = T_Gini, 
             ...
             group = LocName)) +
  geom_line(aes(y = fit.c, color = as.factor(Region)), linewidth = 1) +
  geom_line(aes(y = fit.m), color = "black", linewidth = 1.9) +
  geom_point(shape=16, alpha =0.8, size = 3) +
  scale_color_manual(values = Region_colors_LMM) +
...
# adding significance star to klamath
lab_fun <- function(x) { 
  ifelse(x == "Klamath R.", paste0(x, " \n*"), x) }

residual_plot_facet2 <- ggplot(LMM_Gini_statewide, aes(x = Year, y = Residuals)) +
  geom_point(aes(color = Region), size = 3, alpha = 0.8) +
  geom_hline(aes(yintercept = yintercept, color = Region), linewidth = 1) +
  ...
  facet_wrap(~ LocName, nrow = 2, scales = "fixed", axes = "all", axis.labels = "margins", labeller = labeller(LocName = lab_fun)) +
  theme(panel.border = element_blank(), 
      panel.grid.major = element_blank(),
      panel.grid.minor = element_blank(), 
      axis.line = element_line(colour = "black"), 
...
T_Gini_Flow <- T_Gini_Flow %>% 
  ...
  mutate(Region = case_when ( ## Region classification
    ...
    LocCode %in% 10:14 ~ "Other", 
    LocCode %in% 15:20 ~ "Central Valley"),
    Region = ifelse(is.na(Region), "Statewide", Region))
...
scale_linetype_manual(
  values = c(
    "Log Annual Flow" = "solid",
    "Detrended Gini" = "twodash"
  ),
  labels = c(
    "Detrended Gini",
    expression(Log~Annual~Flow~(m^3/s))
  )
)

flow_plot <- ggplot(T_Gini_Flow, aes(x = Year)) +
   geom_line(aes(y = Residuals, linetype = "Detrended Gini", group = LocName), color = "darkslategray",linewidth = 1) +
  geom_line(aes(y = LogFlow_cms/100, linetype = "Log Annual Flow (m³/s)", group = LocName), color = "darkslategray", linewidth = 1.5) +
  scale_linetype_manual(values = c("Log Annual Flow (m³/s)" = "solid", "Detrended Gini" = "twodash" )) +
  scale_y_continuous(
    name = "Detrended Gini",
    limits = c(-.08, .10),
    sec.axis = sec_axis(~ . *100, name = "Log Annual Flow (m³/s)")) +
  theme_minimal(base_size = 16) +
  scale_x_continuous(breaks = seq(min(2010), max(2025), by = 5),
                     limits =c(2010, 2025)) +
...
# # With region colors: 
# flow_plot2 <- ggplot(T_Gini_Flow, aes(x = Year)) +
#    geom_line(aes(y = Residuals, linetype = "Detrended Gini", group = LocName, color = Region ), linewidth = 1) +
#   geom_line(aes(y = LogFlow_cms/100, linetype = "Log Annual Flow (m³/s)", group = LocName, color = Region), linewidth = 1.5) +
#   scale_linetype_manual(values = c("Log Annual Flow (m³/s)" = "solid", "Detrended Gini" = "twodash" )) +
#    scale_color_manual(values =  Region_colors2) +
#   scale_y_continuous(
#     name = "Detrended Gini",
...
# 
# No_CPUE <- SRRC_filtered_ALL %>%  # Num cards lacking CPUE data
#   filter(is.na(CPUE) | is.infinite(CPUE)) %>% 
#   nrow()
# 
# No_CPUE / nrow(SRRC_filtered_ALL) # Percentage cards lacking CPUE data
# 
# 
...
map_sum_sized <- ca_base +
  ...
  scale_size_area(
    name = "No. Fish (2012–2022)",
    max_size = 11,
    breaks = c(1000, 5000, 20000, 50000, 100000),
    labels = scales::comma) +
